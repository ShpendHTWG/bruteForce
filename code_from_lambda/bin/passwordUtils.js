...
function buildParamsForTime(alphabet, workerAmount, passwordLength, startTime, startProcessingTime, found, password) {
    const entry = {
        Id: { "S": found ? `lambda_${workerAmount}_workers_found_true` : `lambda_${workerAmount}_workers_found_false` },
...
function buildParamsForPercentage(percent, alphabet, passwordLength, startTime, startProcessingTime, workerAmount) {
    ...
    if (percent < 10) {
        entry.Id = { "S": `lambda_0${percent.toString()}%` };
    } else {
        entry.Id = { "S": `lambda_${percent.toString()}%` };
    }
