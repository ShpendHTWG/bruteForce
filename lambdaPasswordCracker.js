...
const { crackPasswordWithParallelAlgorithmInLambda } = require('./bin/lambda');
...
const workerAmount = parseInt(process.argv[4]);
...
async function main() {
    const result = await crackPasswordWithParallelAlgorithmInLambda(alphabet, passwordHash, workerAmount, passwordLength, startTime);
